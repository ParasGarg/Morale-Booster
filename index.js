...
const handlers = {
    ...
    'WelcomeIntent': function () {
        const speechOutput = message.getWelcomeMessage() + " " + message.getHelpMessage();
    ...
    'FightIntent': function() {
        ...
        if (!intentObj.slots.person.value) {
            ...
            const speechOutput = helperFn.getRandomValue(fightData.getPersonSpeakout());
            ...
            const speechOutput = helperFn.getRandomValue(fightData.getReasonSpeakout());
            const updatedIntent = 'FightIntent';
            this.emit(':elicitSlot', slotToElicit, speechOutput, updatedIntent);
        }
    ...
    'InterviewIntent': function() {
        ...
        if (!intentObj.slots.company.value) {
            ...
            const speechOutput = helperFn.getRandomValue(interviewData.getCompanySpeakout());
            const updatedIntent = 'InterviewIntent';
            this.emit(':elicitSlot', slotToElicit, speechOutput, updatedIntent);
        } else if (!intentObj.slots.when.value) {
            // get slot: when
            const slotToElicit = 'when';
            const speechOutput = helperFn.getRandomValue(interviewData.getWhenSpeakout());
            const updatedIntent = 'InterviewIntent';
            this.emit(':elicitSlot', slotToElicit, speechOutput, updatedIntent);
        }
    ...
    'FeelingIntent': function() {
        ...
        if (!intentObj.slots.emotion.value) {
            ...
            const speechOutput = helperFn.getRandomValue(feelingData.getEmotionSpeakout());
        ...
        const suggestion = "I would suggest you to " + helperFn.getFeelingSuggestion(emotion);
